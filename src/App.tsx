...
function App() {
  ...
  return (
    <div className="relative min-h-screen bg-royal-black text-luxury-ivory overflow-hidden selection:bg-luxury-gold selection:text-royal-black w-full">
      ...
      {/* Background audio player (hidden visually) */}
      <MusicPlayer playTriggered={doorsOpened} />
      ...
      <AnimatePresence mode="wait">
        {!doorsOpened ? (
          ...
          >
            <DoorOpening onOpen={() => setDoorsOpened(true)} />
          </motion.div>
        ) : (
          <motion.div
            key="invitation"
            ...
            className="w-full h-screen overflow-y-scroll snap-y snap-mandatory no-scrollbar relative bg-royal-black scroll-smooth"
          >
            ...
            <section className="h-screen w-full snap-start flex flex-col justify-center items-center py-4 px-4 relative z-10 overflow-hidden">
              <InvitationCard />
            </section>
